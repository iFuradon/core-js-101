...
function doRectanglesOverlap(rect1, rect2) {
  const isLeftOf = rect1.left + rect1.width < rect2.left || rect2.left + rect2.width < rect1.left;
  const isAbove = rect1.top + rect1.height < rect2.top || rect2.top + rect2.height < rect1.top;
  return !(isLeftOf || isAbove);
}
...
function findFirstSingleChar(str) {
  const arr = str.split('');
  const f = arr.filter((char) => str.split(char).length - 1 === 1);
  return f.shift() || null;
}
...
function getDigitalRoot(num) {
  const sumDigits = (n) => n.toString().split('').reduce((sum, digit) => sum + Number(digit), 0);
